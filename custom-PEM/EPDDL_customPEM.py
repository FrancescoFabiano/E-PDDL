...
from action_customPEM import Action

class EPDDL_Parser:

    SUPPORTED_REQUIREMENTS = [':strips', ':negative-preconditions', ':typing', ':no-duplicates', ':mep', 'custom-pem']


    def get_domain_name(self, problem_filename):
        try:
            with open(problem_filename,'r') as f:
                # Remove single line comments
                str = re.sub(r';.*$', '', f.read(), flags=re.MULTILINE).lower()
                for t in re.findall(r'\{\s*include\s*\:.+\}', str):
                    return re.sub(r'\{\s*include\s*\:\s*(\S+)\s*\}', r'\1', t)
    ...
    def scan_tokens(self, filename):
        try:
            with open(filename,'r') as f:
                ...
                #Remove the include used to open the right domain file
                str = re.sub(r'\{\s*include\s*\:.+\}', '', str, flags=re.MULTILINE)
    ...
    def parse_obs_groups(self, group):
        for obs_group in group:
            obs_group_name = obs_group.pop(0)
            if obs_group_name in self.obs_groups:
                raise Exception('Observability Group ' + obs_group_name + ' redefined')
            else:
                self.obs_groups.append(obs_group_name)
    ...
    def parse_action(self, group):
        ...
        del_effects = []
        obs = {}
        for ag_group in self.obs_groups:
            obs[ag_group] = []
        derive_cond = []
        explicit_eff = []
        extensions = None
        ...
        self.actions.append(Action(name, act_type, parameters, positive_preconditions, negative_preconditions, add_effects, del_effects, obs, self.obs_groups, derive_cond, explicit_eff, extensions))
